    ...
    private function save($params = array())
    {
        ...
        foreach (gv($params, 'fee_groups') as $fee_group) {
            ...
            foreach ($fee_group['installments'] as $installment) {
                ...
                $late_fee_frequency = gbv($installment, 'late_fee_applicable');
                $fee_installment->title = $installment['title'];
                $fee_installment->due_date = toDate($installment['due_date']);
                $fee_installment->late_fee_applicable = $late_fee_frequency;
                $fee_installment->late_fee_frequency = $late_fee_frequency ? gv($installment, 'late_fee_frequency', 0) : 0;
                $fee_installment->late_fee = $late_fee_frequency ? gv($installment, 'late_fee', 0) : 0;
