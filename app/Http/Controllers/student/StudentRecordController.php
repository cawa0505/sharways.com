...
class StudentRecordController extends Controller
{
    ...
    public function printReceipt($uuid, $record_id, $fee_record_id, $transaction_id)
    {
        ...
        $transaction = $this->repo->getReceipt($student_record, $fee_record_id, $transaction_id);

        $student_record->load(['student','student.parent','admission','batch','batch.course']);

        $transaction_group = \App\Models\Finance\Transaction\Transaction::with(['transactionGroups' => function($q) {
        ...
        $transactions = \App\Models\Finance\Transaction\Transaction::whereIn('id', $transaction_group_ids)->with('studentFeeRecord','studentFeeRecord.studentFeeRecordDetails','studentFeeRecord.studentFeeRecordDetails.transaction','studentFeeRecord.feeConcession','studentFeeRecord.feeConcession.feeConcessionDetails')->get();

        return view('print.student.fee-receipt', compact('transaction', 'student_record', 'transactions'))->render();
    }
