    ...
    <table class="fancy-detail">
        ...
        <tbody>
            @foreach($students as $student)
                <tr>
                    @if(isColumnVisible('admission_number', $filter))
                        <td>{{$student->StudentRecords->first()->Admission->admission_number}}</td>
                    @endif
                    @if(isColumnVisible('roll_number', $filter))
                        <td>{{getRollNumber($student->StudentRecords->first())}}</td>
                    @endif
                    ...
                    @endif
                    @if(isColumnVisible('date_of_admission', $filter))
                        <td>{{showDate($student->StudentRecords->first()->Admission->date_of_admission)}}</td>
                    @endif
                    @if(isColumnVisible('date_of_promotion', $filter))
                        <td>
                            @if($student->StudentRecords->first()->Admission->date_of_admission == $student->StudentRecords->first()->date_of_entry)
                                <span>-</span>
                            @else
                                <span>{{showDate($student->StudentRecords->first()->date_of_entry)}}</span>
                            @endif
                        </td>
                    @endif
                    @if(isColumnVisible('contact_number', $filter))
                    ...
                    @endif
                        <td>{{$student->StudentRecords->first()->Batch->Course->name}}</td>
                        <td>{{$student->StudentRecords->first()->Batch->name}}</td>
                    @if(isColumnVisible('nationality', $filter))
