@include('print.print-layout.header',compact('print_options'))
    ...
    <table class="report-card" style="margin-top: 20px;">
		<tr>
			...
			<td rowspan="2" style="font-weight: bold;">{{trans('student.admission_number_short')}}</td>
		...
		@foreach(gv($data, 'rows') as $row)
			<tr>
				...
				<td>{{gv($row, 'admission_number')}}</td>
				@foreach(gv($row, 'marks') as $mark)
					<td style="text-align: center;">{{$mark}}</td>
				@endforeach
				<td style="text-align: center;">{{gv($row, 'total')}}/{{gv($row, 'max_mark')}}</td>
